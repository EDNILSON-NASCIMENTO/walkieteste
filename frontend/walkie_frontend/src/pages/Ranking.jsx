...
const Ranking = () => {
  ...
  const getImageUrl = (imagePath) => {
    ...
    const fullUrl = `${baseUrlWithoutApi.replace(
      /\/$/,
      ""
    )}/${imagePath.replace(/^\//, "")}`;
    return fullUrl;
  ...
  const fetchData = async (period) => {
    ...
    try {
      ...
      setRankingData(
        rankingRes.data || { ranking: [], current_user_position: null }
      );
  ...
  const getRankIcon = (position) => {
    switch (position) {
      ...
      case 3:
        return (
          <Award className="w-5 h-5 sm:w-6 sm:h-6 text-amber-600 flex-shrink-0" />
        );
      default:
        return (
          <span className="w-6 sm:w-8 h-6 sm:h-8 flex items-center justify-center text-xs sm:text-sm font-bold text-gray-600 flex-shrink-0">
            {position}
          </span>
        );
    }
  ...
  const formatChallengeProgress = (challenge) => {
    ...
    if (challenge.type === "daily") {
      ...
      progressText = `${(progress / 1000).toFixed(1)} / ${(
        target / 1000
      ).toFixed(1)} km`;
    } else {
      progressText = `${progress}/${target}`;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          ...
          <p className="text-gray-600 mt-1 text-sm sm:text-base">
            Veja sua posição e conquiste novos badges
          </p>
        </div>
      </div>
      ...
      {!loading && !error && (
        <Tabs defaultValue="ranking">
          <TabsList className="grid w-full grid-cols-3 h-auto mb-4">
            ...
            <TabsTrigger value="ranking" className="py-2 text-xs sm:text-base">
              Ranking
            </TabsTrigger>
            <TabsTrigger value="badges" className="py-2 text-xs sm:text-base">
              Badges
            </TabsTrigger>
            <TabsTrigger
              value="challenges"
              className="py-2 text-xs sm:text-base"
            >
              Desafios
            </TabsTrigger>
          </TabsList>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex flex-wrap gap-1 w-full sm:w-auto">
                <Button
                  ...
                  className="flex-grow sm:flex-none text-xs sm:text-sm"
                >
                  {" "}
                  Semanal{" "}
                </Button>
                ...
                <Button
                  variant={
                    selectedPeriod === "all_time" ? "default" : "outline"
                  }
                  size="sm"
                ...
                rankingData.ranking.length > 0 ? (
                  <div className="space-y-2 sm:space-y-3">
                          ...
                          <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-blue-500 to-green-500 rounded-full flex items-center justify-center overflow-hidden border">
                            ...
                            {user.profile_picture ? (
                              <img
                                ...
                                key={getImageUrl(user.profile_picture)}
                                onError={(e) => {
                    ...
                    {rankingData.current_user_position &&
                      !rankingData.ranking.some((u) => u.is_current_user) && (
          ...
          <TabsContent value="badges" className="space-y-4 md:space-y-6">
            <Card>
              <CardHeader className="pb-3 sm:pb-4">
                <CardTitle className="flex items-center text-base sm:text-lg">
                  <Award className="w-4 h-4 sm:w-5 sm:h-5 mr-2" /> Minhas
                  Conquistas
                </CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      >
                        ...
                        <h3
                          className={`font-semibold text-sm sm:text-base mb-1 ${
                            badge.earned ? "text-yellow-700" : "text-gray-700"
                          }`}
                        >
                          {badge.name}
                        </h3>
          ...
          <TabsContent value="challenges" className="space-y-4 md:space-y-6">
            <Card>
              <CardHeader className="pb-3 sm:pb-4">
                <CardTitle className="flex items-center text-base sm:text-lg">
                  <Target className="w-4 h-4 sm:w-5 sm:h-5 mr-2" /> Desafios
                  Ativos
                </CardTitle>
              </CardHeader>
              <CardContent>
